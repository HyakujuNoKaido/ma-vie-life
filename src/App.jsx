...
try {
  ...
  if (!firebaseConfig || !firebaseConfig.apiKey) configError = "Config manquante.";
} catch (e) { configError = "Erreur JSON."; }
...
const Button = ({ children, onClick, variant = "primary", className = "" }) => {
  const styles = {
    ...
    ghost: "text-slate-300 hover:text-red-500"
  };
  return (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      if (!u) await signInAnonymously(auth).catch(e => setError(e.message));
      else setUser(u);
      setLoading(false);
    });
    return () => unsub();
  ...
  const ViewAccueil = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex justify-between items-end">
        <div><h1 className="text-4xl font-black tracking-tighter">LIFE.</h1><p className="text-slate-500 font-medium text-sm">Bonjour 👋 Votre dashboard 🇨🇭</p></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="bg-indigo-600 text-white border-none shadow-xl">
          ...
          <p className="text-[10px] text-indigo-100 font-bold mt-2 uppercase opacity-80">{budgetGoal > 0 ? `Objectif : ${budgetGoal} CHF` : "Pas d'objectif fixé"}</p>
        </Card>
        <Card className="flex flex-col justify-between border-slate-100">
          ...
          <div className="flex gap-1 overflow-hidden mt-2">
            {subscriptions.slice(0,3).map(s => <span key={s.id} className="text-[8px] bg-slate-50 px-2 py-1 rounded-full font-bold text-slate-400 uppercase">{s.name}</span>)}
          </div>
        </Card>
      </div>
    </div>
  ...
  const ViewBudget = () => {
    ...
    const handleAdd = async () => {
      ...
      await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, col), {
        amount: form.amount, name: form.label || form.cat, category: form.cat, 
        day: form.day, date: new Date().toLocaleDateString('fr-CH'), createdAt: Date.now()
      });
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-2 bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100">
          ...
          <button onClick={() => setSubTab('ajouter')} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase ${subTab === 'ajouter' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400'}`}>Ajouter</button>
          <button onClick={() => setSubTab('config')} className={`px-4 rounded-xl text-xs font-black ${subTab === 'config' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400'}`}><Target size={18}/></button>
        </div>
        ...
        {subTab === 'config' && (
          <Card className="bg-indigo-50 border-indigo-100">
            ...
            <div className="flex gap-2">
              <input type="number" className="flex-1 p-4 rounded-2xl border-none font-black text-lg outline-none shadow-inner" placeholder="CHF" value={budgetGoal || ''} onChange={e => setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'settings', 'budget'), { monthlyGoal: Number(e.target.value) })} />
        ...
        {subTab === 'ajouter' && (
          <Card className="animate-in slide-in-from-bottom-4">
            <div className="space-y-4">
              ...
              <Button className="w-full py-4 text-sm font-black uppercase" onClick={handleAdd}>Valider l'entrée</Button>
            </div>
          </Card>
        )}
        ...
        {subTab === 'lib' ? (
          <div className="grid grid-cols-2 gap-3 pb-10">
            {STATIC_EXERCICES.map(ex => (
              <div key={ex.id} className="bg-white rounded-[32px] overflow-hidden border border-slate-50 shadow-sm">
          ...
          <div className="space-y-4">
            {workouts.map(w => (
              <Card key={w.id} className="border-l-8 border-indigo-600 shadow-sm">
                <div className="flex justify-between items-start mb-4"><div><h3 className="text-xl font-black">{w.sessionName}</h3><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{w.date}</p></div><Button variant="ghost" onClick={() => deleteItem('workouts', w.id)}><Trash2 size={16}/></Button></div>
              </Card>
            ))}
          </div>
        )}
  ...
  const ViewNutrition = () => (
    <div className="space-y-6">
      ...
      <Card><p className="text-slate-400 text-sm italic py-4 text-center">Planifiez vos menus ici.</p></Card>
  ...
  const ViewAgenda = () => (
    <div className="space-y-6">
      ...
      <Card><p className="text-slate-400 text-sm italic py-4 text-center">Gérez vos priorités du jour.</p></Card>
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4 font-sans text-center px-6">
      <div className="w-16 h-16 bg-indigo-600 rounded-[30px] animate-bounce flex items-center justify-center text-white text-2xl font-black">L</div>
...
const NavItem = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all ${active ? 'bg-indigo-600 text-white shadow-xl font-bold' : 'text-slate-400 hover:text-slate-900'}`}>
    {icon} {label}
  </button>
);

const MobileItem = ({ active, onClick, icon }) => (
  <button onClick={onClick} className={`p-4 rounded-[24px] transition-all ${active ? 'bg-indigo-600 text-white shadow-xl -translate-y-4 scale-110' : 'text-slate-300'}`}>
    {icon}
  </button>
);

// --- MOUNT ---
if (typeof window !== 'undefined' && !window.__is_gemini_preview) {
