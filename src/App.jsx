...
/**
 * LIFE Dashboard - Version 5.5 (Ultra-Sécurisée)
 * Tout est encapsulé pour éviter l'écran blanc.
 */
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (rawConfig) {
          try {
            // Nettoyage des guillemets et espaces invisibles
            const cleaned = rawConfig.trim().replace(/^['"]|['"]$/g, '');
            config = JSON.parse(cleaned);
          } catch (e) {
            throw new Error("Le texte dans VITE_FIREBASE_CONFIG n'est pas un JSON valide. Vérifiez qu'il n'y a pas de guillemets autour du bloc sur Vercel.");
          }
        ...
        if (!config || !config.apiKey) {
          throw new Error("Configuration Firebase introuvable. L'application est prête mais attend ses clés sur Vercel.");
        }

        // Démarrage Firebase (uniquement si pas déjà fait)
        ...
        onAuthStateChanged(firebaseAuth, (u) => {
          if (!u) {
            signInAnonymously(firebaseAuth).catch(err => setError("Accès Cloud refusé : " + err.message));
          } else {
            setUser(u);
            setLoading(false);
          }
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 font-sans">
      <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-red-100 max-w-md text-center">
        ...
        <h2 className="text-2xl font-black text-slate-800 mb-4">Erreur de Configuration</h2>
        <p className="text-sm text-red-500 mb-8 font-medium leading-relaxed">{error}</p>
        <div className="text-left bg-slate-50 p-6 rounded-3xl text-[10px] font-mono text-slate-400 mb-8 leading-relaxed">
          <strong>Solution :</strong><br/>
          1. Copiez la ligne dans <em>config_pure.json</em> (à droite).<br/>
          2. Sur Vercel, éditez <em>VITE_FIREBASE_CONFIG</em>.<br/>
          3. Supprimez tout et collez la ligne.<br/>
          4. Faites un "Redeploy" manuel.
        </div>
        <button onClick={() => window.location.reload()} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs">Vérifier à nouveau</button>
      </div>
    </div>
  );
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4 font-sans">
      <div className="w-16 h-16 bg-indigo-600 rounded-[32px] animate-bounce flex items-center justify-center text-white font-black text-2xl shadow-2xl">L</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-28 md:pb-0 md:pl-72 font-sans antialiased">
      <aside className="fixed left-0 top-0 bottom-0 w-72 bg-white border-r border-slate-100 p-8 hidden md:flex flex-col shadow-sm">
        <div className="text-3xl font-black mb-12 text-indigo-600 flex items-center gap-3">
        ...
        {activeTab === 'accueil' ? (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex justify-between items-end">
              <div><h1 className="text-4xl font-black tracking-tighter">Dashboard</h1><p className="text-slate-500 font-medium text-sm">Bonjour 👋 État de vos comptes</p></div>
